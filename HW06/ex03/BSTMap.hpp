...
class	BSTMap : public ASortedMap<K, V>
{
	...
	public:
		...
		TreeNode		*ft_separate_root()
		{
			return (this->_tree.root());
		}
		void	draw_tree()
		{
			ft_draw_node(this->ft_separate_root(), 0);
		}
	private:
		void	ft_draw_node(TreeNode *root, int offset)
		{
			if (!root)
				return ;
...
template <typename K, typename V>
static void	ft_bst_median_inorder(typename LinkedBinaryTree<std::unique_ptr<typename IMap<K, V>::Entry>>::Node *root,
	ArrayVector<typename LinkedBinaryTree<std::unique_ptr<typename IMap<K, V>::Entry>>::Node*> &container,
	int	&size)
{
	if (root->left())
		ft_bst_median_inorder<K, V>(root->left(), container, size);
	if ((**root).get())
	{
		container.insert(container.size(), root);
		size++;
	}
	if (root->right())
		ft_bst_median_inorder<K, V>(root->right(), container, size);
}
...
typename BSTMap<K, V>::Entry	*ft_bst_median(BSTMap<K, V> &tree)
{
	if (tree.empty())
		return (nullptr);
	ArrayVector<typename BSTMap<K, V>::TreeNode*>	container;
	int	size = 0;
	ft_bst_median_inorder<K, V>(tree.ft_separate_root(), container, size);
	size /= 2;
